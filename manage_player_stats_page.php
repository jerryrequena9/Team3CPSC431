<?php
    ...
    function display_player() {
        ...
        if (isset($_POST['manage_player_stats_player_id'])) {
            display_player_stats($selected_player, $game_id);
        }
    }

    function display_player_stats($player_id, $game_id) {
        $query = "
            SELECT
                p.first_name,
                p.last_name,
        ...
        global $db;
        $stmt = prepare_with_perms($db, $query);
        if (!$stmt->bind_param('ii', $game_id, $player_id) || !$stmt->execute()) {
            display_error_exit("failed to fetch player stats");
        }
        ...
        if ($result->num_rows > 0) {
            while ($stat = $result->fetch_assoc()) {
                ...
                foreach ($fields as $field) {
                    ...
                    echo "<td>
                            <form method='POST' action='manage_player_stats_page.php'>
                                <input type='hidden' name='manage_player_stats_team_id' value='" . intval($_POST['manage_player_stats_team_id'])   . "'>
                                <input type='hidden' name='manage_player_stats_season_id' value='" . intval($_POST['manage_player_stats_season_id']) . "'>
                                <input type='hidden' name='manage_player_stats_game_id' value='" . intval($_POST['manage_player_stats_game_id'])   . "'>
                                <input type='hidden' name='manage_player_stats_player_id' value='" . intval($_POST['manage_player_stats_player_id']) . "'>
                                <p>$val</p>
            ...
            echo "No stats are available for this player in this game.";
        }
        echo "</table>";
    }

    function handle_stat_edit() {
